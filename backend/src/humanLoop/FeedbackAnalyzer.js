...
class FeedbackAnalyzer {
  ...
  generateRecommendations(analysis, user) {
    ...
    if (analysis.avgRating < 3) {
      recommendations.push({
        type: "overall_low",
        message:
          "Your overall satisfaction is low. Consider updating your interests or keywords.",
  ...
  async getSystemWideFeedback() {
    ...
    const eventStats = Object.values(eventRatings).map(
      ({ event, ratings }) => ({
        event,
